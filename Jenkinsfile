            ...
            steps {
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'docker-private', passwordVariable: 'PASSWORD', usernameVariable: 'USERNAME')]){
                        sh 'docker build -t pm310/simple-app:ra-3.0 .'
                        sh 'docker login -u $USERNAME -p $PASSWORD'
                        sh 'docker push pm310/simple-app:ra-3.0'
                    }
                }
            }
        }
        stage("deploy") {
            steps {
                script {
                    ...
                    def dockerCmd="docker run -p 80:80 --name ec2-index -d pm310/simple-app:ra-3.0"
